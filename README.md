...
* **Usage:** Run `python generate_drift_data.py` to recreate the drift dataset. You can feed this dataset to the batch prediction endpoint to see how the model behaves when given extreme outliers. 
...
* **Usage:** Run `sh test_api.sh` to quickly verify that both the Backend API and the Nginx routing are functioning properly.

### `services/imitation.py`
...
* **Usage:** Use this to fill **Grafana** with live data: run `python services/imitation.py`.
